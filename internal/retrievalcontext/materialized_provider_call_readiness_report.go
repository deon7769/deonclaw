...
func LoadMaterializedProviderCallReadinessReport(path string) (MaterializedProviderCallReadinessReportResult, []byte, error) {
	if err := validateRelativeSafePath("readiness report path", path); err != nil {
		return MaterializedProviderCallReadinessReportResult{}, nil, err
	}
	data, err := os.ReadFile(path)
	if err != nil {
		return MaterializedProviderCallReadinessReportResult{}, nil, fmt.Errorf("read materialized provider call readiness report %q: %w", path, err)
	}
	return ParseMaterializedProviderCallReadinessReportJSON(data)
}

func ParseMaterializedProviderCallReadinessReportJSON(data []byte) (MaterializedProviderCallReadinessReportResult, []byte, error) {
	if strings.Contains(string(data), "text_excerpt") || strings.Contains(string(data), "alpha text") {
		return MaterializedProviderCallReadinessReportResult{}, nil, fmt.Errorf("materialized provider call readiness report must not contain materialized preview text")
	}
	var result MaterializedProviderCallReadinessReportResult
	if err := json.Unmarshal(data, &result); err != nil {
		return MaterializedProviderCallReadinessReportResult{}, nil, fmt.Errorf("parse materialized provider call readiness report json: %w", err)
	}
