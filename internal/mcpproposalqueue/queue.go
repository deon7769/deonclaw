...
type ShowDetail struct {
	...
	ProposalPath          string   `json:"proposal_path,omitempty"`
	LintPath              string   `json:"lint_path,omitempty"`
	PreflightPath         string   `json:"preflight_path,omitempty"`
	ArtifactPath          string   `json:"artifact_path,omitempty"`
	LintArtifactPath      string   `json:"lint_artifact_path,omitempty"`
...
type ExportResult struct {
	...
	ProposalSHA256 string `json:"exported_proposal_sha256"`
...
func Show(ctx context.Context, db store.Store, runID string) (ShowDetail, error) {
	...
	var parsedProposal mcpapproval.MCPToolCallProposal
	if hasProposal {
		detail.ProposalPath = proposalPath
	...
	if lintPath, ok := artifactPaths[ProposalLintArtifactName]; ok {
		...
		detail.LintArtifactPath = lintPath
	...
	if preflightPath, ok := artifactPaths[ProposalPreflightArtifactName]; ok {
		...
		detail.PreflightArtifactPath = preflightPath
	...
	detail.ReadyForApproval = readyForApproval(detail, proposalParsed)
	if proposalParsed && !detail.ApprovalRefused {
		detail.Suggested = suggestedCommands(proposalPath, parsedProposal)
	}

	return detail, nil
}

func Export(ctx context.Context, db store.Store, runID string, outputPath string) (ExportResult, error) {
	runID = strings.TrimSpace(runID)
	outputPath = strings.TrimSpace(outputPath)
	if runID == "" {
		return ExportResult{}, fmt.Errorf("run id is required")
	}
	if outputPath == "" {
	...
	if !ok {
		return ExportResult{}, fmt.Errorf("run %q has no MCP tool call proposal artifact", runID)
	}
	content, err := os.ReadFile(proposalPath)
	if err != nil {
		return ExportResult{}, fmt.Errorf("read proposal artifact %q: %w", proposalPath, err)
	}
	...
	if err := os.WriteFile(outputPath, content, 0o600); err != nil {
		return ExportResult{}, fmt.Errorf("write proposal export %q: %w", outputPath, err)
	}

	result := ExportResult{
		OutputPath:     outputPath,
		ProposalSHA256: fileContentSHA256(content),
	}
	detail, err := Show(ctx, db, runID)
	if err == nil {
		result.NextStepHint = nextStepHint(detail, outputPath)
	}
	return result, nil
}

func readyForApproval(detail ShowDetail, proposalParsed bool) bool {
	if !proposalParsed || detail.ApprovalRefused {
		return false
	}
	if detail.ProposalStatus != StatusPreflightPassed {
		return false
	}
	return strings.TrimSpace(detail.ProposalID) != "" &&
		strings.TrimSpace(detail.Server) != "" &&
		strings.TrimSpace(detail.Tool) != ""
}

func nextStepHint(detail ShowDetail, exportedPath string) string {
	if detail.ApprovalRefused {
		return "Worker approval artifact was refused. Do not approve or execute this proposal."
	}
	if detail.ReadyForApproval {
		return fmt.Sprintf(
			"Proposal passed preflight and is ready for manual approval. Next: deonctl mcp proposal approve --proposal %s ... then deonctl mcp proposal execute --confirm-execute",
			exportedPath,
		)
	}
	configPath := strings.TrimSpace(detail.ConfigPath)
	if configPath == "" {
		configPath = "<mcp.yaml>"
	}
	policyPath := strings.TrimSpace(detail.PolicyPath)
	if policyPath == "" {
		policyPath = "<policy.yaml>"
	}
	return fmt.Sprintf(
		"Review violations/preflight failures, then rerun: deonctl mcp proposal preflight --proposal %s --config %s --policy %s --output <preflight.json>",
		exportedPath,
		configPath,
		policyPath,
	)
...
func WriteListText(result ListResult, out io.Writer) error {
	...
	if _, err := fmt.Fprintln(table, "run_id\ttask_id\tworker\tproposal_status\tready_for_approval\tproposal_sha256\tproposal_id\tserver\ttool\tpreflight_status\tproposal_path"); err != nil {
		return err
	}
...
func WriteShowText(detail ShowDetail, out io.Writer) error {
	lines := []struct {
		...
		{"ready_for_approval", fmt.Sprintf("%t", detail.ReadyForApproval)},
		...
		{"proposal_path", detail.ProposalPath},
		{"lint_path", detail.LintPath},
		{"preflight_path", detail.PreflightPath},
	}
	for _, line := range lines {
		if line.name == "ready_for_approval" {
...
func buildListEntry(ctx context.Context, db store.Store, runRecord runs.Run) (ListEntry, bool, error) {
	...
	entry.ReadyForApproval = readyForApproval(ShowDetail{
		ProposalStatus:  entry.ProposalStatus,
		ProposalID:      entry.ProposalID,
		Server:          entry.Server,
		Tool:            entry.Tool,
		ApprovalRefused: entry.ApprovalRefused,
	}, proposalParsed)
...
func suggestedCommands(proposalPath string, proposal mcpapproval.MCPToolCallProposal) []string {
	configPath := strings.TrimSpace(proposal.ConfigPath)
	if configPath == "" {
		configPath = "<mcp.yaml>"
	}
	policyPath := strings.TrimSpace(proposal.PolicyPath)
