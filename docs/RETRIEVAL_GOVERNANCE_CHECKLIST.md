...
- `make provider-call-chain-smoke` runs library + CLI fixture e2e guards (Tasks 22.37–22.64) with hash coherence and anti-leak assertions
...
- provider call executor preflight reconciles hashes and keeps `execution_allowed_now: false`
...
- provider execution simulation keeps `execution_result_available: false` and all execution flags false
