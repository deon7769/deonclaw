...
| 22.18 | Materialized prompt section preview (`retrieval context prompt-preview`), no worker execution |
| 22.18.1 | Prompt preview report/QA (`retrieval context prompt-preview-report`), no worker execution |
| 22.18.2 | Injection governance release bundle (`retrieval context injection-governance-bundle`), no worker execution |
| 22.19 | Task schema declaration for materialized injection (`retrieval_context.materialized_injection`), no runner execution |
| 22.19.1 | Materialized injection task declaration report (`task materialized-injection-report`), no runner execution |
| 22.20 | Runner materialized injection preflight (`worker codex materialized-injection-preflight`), no worker execution |
| 22.21 | Runner materialized injection dry-run prompt artifact (`worker codex materialized-injection-dry-run`), no worker execution |
| 22.21.1 | Runner materialized injection dry-run report (`worker codex materialized-injection-dry-run-report`), no worker execution |
| 22.21.2 | Runner materialized injection readiness report (`worker codex materialized-injection-readiness-report`), no worker execution |
| 22.22 | Runner materialized injection execution gate (`worker codex materialized-injection-execution-gate`), no worker execution |
| 22.23 | Materialized prompt assembly dry-run (`worker codex materialized-prompt-assembly-dry-run`), no worker execution |
| 22.24 | Materialized prompt assembly report (`worker codex materialized-prompt-assembly-report`), no worker execution |
| 22.25 | Materialized injection runtime config schema (`worker codex materialized-injection-runtime validate`), no worker execution |
| 22.26 | Materialized injection run planner (`worker codex materialized-injection-run-plan`), no worker execution |
| 22.27 | Materialized injection execution enablement policy (`worker codex materialized-injection-execution-enable validate`), no worker execution |
| 22.28 | Materialized injection provider run-plan (`worker codex materialized-injection-provider-run-plan`), no provider call |
| 22.29 | Materialized provider dispatch validate (`worker codex materialized-provider-dispatch validate`), no provider call |
| 22.30 | Materialized provider payload dry-run (`worker codex materialized-provider-payload-dry-run`), no provider call |
| 22.31 | Materialized provider payload report (`worker codex materialized-provider-payload-report`), no provider call |
| 22.32 | Materialized provider call gate (`worker codex materialized-provider-call-gate`), no provider call |
| 22.33 | Materialized provider call readiness report (`worker codex materialized-provider-call-readiness-report`), no provider call |
| 22.34 | Provider call approval (`worker codex provider-call-approval new/approve/inspect`), no provider call |
| 22.35 | Provider call execution bundle (`worker codex provider-call-execution-bundle`), no provider call |
| 22.36 | Provider call chain continuity audit (`worker codex provider-call-chain-audit`), no provider call |
| 22.37 | Provider call chain fixture smoke / CI guard (`scripts/provider-call-chain-fixture-smoke.sh`), no provider call |
| 22.38 | Provider call executor skeleton (`worker codex provider-call-executor validate/plan`), no provider call |
| 22.39 | Provider call executor policy config (`provider-call-executor.yaml`, `config validate`), no provider call |
| 22.40 | Provider call executor dry-run contract (`dry-run`, `dry-run-report`), no provider call |
| 22.41 | Provider call executor preflight, no provider call |
| 22.42 | Provider executor dispatch approval, no provider call |
| 22.43 | Provider transport plan (blocked), no provider call |
| 22.44 | Provider executor release bundle / activation gate, no provider call |
| 22.45 | Provider request envelope dry-run/report, no provider call |
| 22.46 | Provider adapter registry / capability plan (blocked), no provider call |
| 22.47 | Provider response fixture (fake), no provider call |
| 22.48 | Provider execution simulation bundle/report, no provider call |
| 22.49 | Provider credential policy validate/plan, no secret reads |
| 22.50 | Provider real-call proposal new/inspect, not executable |
| 22.51 | Provider response change proposal/report, fixture only |
| 22.52 | Provider activation readiness audit/report, no provider call |
| 22.53 | Provider activation policy validate/plan, blocked |
| 22.54 | Provider activation approval new/approve/inspect, future only |
| 22.55 | Provider activation rehearsal/report, metadata-only sequence |
| 22.56 | Provider activation release package/gate |
| 22.57 | Provider activation chain hardening / anti-leak helpers |
| 22.58 | Provider activation operator review bundle/report |
| 22.59 | Provider activation kill-switch manifest |
| 22.60 | Provider activation final audit / CI report, last gate before dispatch design |
| 22.61–22.64 | Provider real dispatch design package (secret-read proposal, transport plan, dispatch design, design review gate), no execution |
| 22.65–22.68 | Provider real dispatch external approval package (external approval, runbook, risk register, preimplementation gate), no execution |

22.3.1 validates fake vector artifacts read-only. Task 22.4 plans LanceDB writes from validated embedding artifacts. Task 22.4.1 emits fake row/manifest artifacts for table-shape smoke. Task 22.5 writes a real local LanceDB table via a Python adapter. Task 22.6 validates write-smoke databases structurally via doctor/report. Task 22.7 runs controlled top-k vector search with an explicit query vector or existing chunk row after doctor passes. Task 22.7.1 validates search-smoke result artifacts. Task 22.8 attaches validated search metadata passively into worker prompts. Task 22.8.1 inspects retrieval-context artifacts and reports runs with passive attachments. Task 22.9 materializes governed chunk text excerpts from chunks JSONL with explicit confirmation. Task 22.9.1 validates materialized artifacts and hardens materialize input paths. Task 22.10 consolidates retrieval and materialized artifacts into an audit bundle without text. Task 22.11 adds explicit approval for future governed materialized context use without runner injection. Task 22.12 plans future runner injection without execution. Task 22.12.1 consolidates governance chain reporting without injection. Task 22.12.2 adds a reproducible end-to-end fixture and tests. Task 22.13 adds a release/operability checklist without code execution changes. Task 22.14 records the runner injection design decision without implementation. Task 22.15 adds injection policy schema validate/plan without execution. Task 22.15.1 extends the governance e2e fixture with injection-policy validate/plan over real chain artifacts. Task 22.16 adds runner injection approval artifacts without execution. Task 22.17 adds runner injection execution-plan artifacts without worker execution. Task 22.18 adds materialized prompt section preview render without worker execution. Task 22.18.1 adds prompt preview report/QA without worker execution. Task 22.18.2 adds injection governance release bundle consolidation without worker execution. Task 22.19 adds task YAML declaration for future materialized injection with governance bundle validation only. Task 22.19.1 adds materialized injection task declaration report/QA without runner execution. Task 22.20 adds runner materialized injection preflight without worker execution. Task 22.21 adds runner materialized injection dry-run prompt artifact without worker execution. Task 22.21.1 adds runner materialized injection dry-run report/QA without worker execution. Task 22.21.2 adds runner materialized injection readiness report without worker execution. Task 22.22 adds runner materialized injection execution gate without worker execution. Task 22.23 adds materialized prompt assembly dry-run adapter without worker execution and without normal runner prompt changes. Task 22.24 adds materialized prompt assembly report/QA without worker execution. Task 22.25 adds materialized injection runtime config schema validation without worker execution. Task 22.26 adds materialized injection run planner without worker execution and without normal runner prompt changes. Task 22.27 adds materialized injection execution enablement policy schema validation without worker execution. Task 22.28 adds materialized injection provider run-plan without provider call. Tasks 22.29–22.33 add materialized provider dispatch validate, payload dry-run/report, call gate, and readiness report without provider call. Task 22.34 adds provider call approval without provider call. Task 22.35 adds provider call execution bundle without provider call, network, or worker execution. Task 22.36 adds provider call chain continuity audit reconciling producer commands and loaders without provider call. Task 22.37 adds fixture smoke / CI guard over the full provider-call chain with hash and anti-leak assertions without provider call. See docs/MEMORY_EMBEDDINGS.md, docs/MEMORY_LANCEDB.md, docs/RETRIEVAL_CONTEXT.md, docs/RETRIEVAL_GOVERNANCE_CHECKLIST.md, docs/PROVIDER_CALL_CHAIN_GATE.md, and docs/ADR_RETRIEVAL_RUNNER_INJECTION.md.
