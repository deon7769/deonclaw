...
Do not implement fallback execution, Codex Docker worker execution, MCP execution/manager, runner memory retrieval, LanceDB writes, or UI out of order. The current fallback surface is schema/policy only. The current Docker surface is runtime config validation, docker-plan, simple `runtime docker-exec`, Docker-backed validation commands, allowlisted env passthrough, fake/test worker runtime scaffold, MCP fake/test smoke, fake read-only tool-smoke, real read-only discovery, real read-only call-smoke, MCP call approval workflow with stale-hash checks and execution bundle, passive MCP context attachments, worker-generated MCP proposal lint/preflight, run-scoped MCP proposal review queue (`mcp proposals list/show/export`), memory index foundation (`memory index validate/plan/build`), and OpenCode Docker smoke only; Codex still runs locally. The current MCP surface is registry validation/list/plan/doctor/risk/docker-plan plus local/Docker fake/test stdio smoke, fake read-only tool-smoke, policy-gated real read-only discovery, policy-gated one-call real read-only call-smoke, explicit proposal/preflight/approval execution for that same call path, passive runner context attachments, worker proposal validation only, and run-scoped proposal review queue without execution; no automatic MCP tool orchestration, worker tool execution, or fallback integration. The current memory index surface is config validation, planning, and auditable chunk build only; no embeddings, LanceDB, or runner retrieval yet.
...
- internal/mcpapproval: MCP read-only tool call proposal, lint, preflight, approval, hash binding, stale approval rejection, execution bundle, and explicit execute workflow only
- internal/mcpcontext: passive validation and summarized markdown for audited MCP discovery/call artifacts only
- internal/mcpproposalqueue: read-only review queue over persisted run MCP proposal artifacts; list/show/export only, no MCP server startup or tool calls
- internal/runner: worker-generated MCP tool call proposals are linted/preflighted after worker exit only; runner must not start MCP servers or execute tools
...
- internal/memoryindex: memory index config validation, planning, and auditable chunk build only; no embeddings, LanceDB, or runner retrieval
