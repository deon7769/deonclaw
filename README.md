...
**23.24–23.25 in progress:** manual `work dispatch-once --mode real --confirm-worker-dispatch` now reaches Codex/OpenCode runner adapters only after lease, budget, task snapshot and skill snapshot gates, then persists worker events and artifacts under the active dispatch run. CI remains fake-only; lease renewal and cancellation hardening continue in `23.26–23.27`.

**Next:** continue real Codex/OpenCode dispatch hardening (`23.26–23.27`). Real skill registry apply from learning proposals remains gated behind a later explicit integration. See [Epic 23 Roadmap](docs/EPIC_23_ROADMAP.md).
