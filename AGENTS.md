...
- 23.24-23.25 real dispatch adapter foundation: manual `work dispatch-once --mode real --confirm-worker-dispatch` reaches Codex/OpenCode worker adapters only after lease, budget, task snapshot and skill snapshot gates, reports provider/network boundaries, and persists worker events/artifacts under the active dispatch run; CI smokes remain fake-only and lease renewal/cancellation hardening is still next
